...
describe('InMemoryConfigRepository', () => {
  it('keeps one live override and preserves immutable versions', async () => {
    const repository = new InMemoryConfigRepository({ agents: [{ id: 'agent', baseConfig: base, activeVersionId: null, writablePaths: ['rules'], maxDiffBytes: 4_096, signingKey: 'key' }] })
