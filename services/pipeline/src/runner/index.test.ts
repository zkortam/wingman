  ...
  it('runs five genuinely distinct intercepted samples concurrently', async () => {
    const result = await runAssertion({
      runner: new StubRunner(({ sample }) => ({
        toolCalls: sample === 0 ? [] : [{ id: String(sample), name: 'search', args: {} }],
      })),
      assertion,
    ...
    // One cassette key holds five recorded responses, so the key is stable and it is the decisions.
    expect(new Set(result.results.map(({ cassetteKey }) => cassetteKey)).size).toBe(1)
    expect(
      new Set(result.results.map(({ toolCalls }) => JSON.stringify(toolCalls))).size,
    ).toBeGreaterThan(1)
