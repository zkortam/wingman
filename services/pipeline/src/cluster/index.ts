...
import { incidentKey, type EvidenceExcerpt, type Signal } from '@wingman/schema'
...
export function evidenceExcerpt(session: ObservedSession, signal: Signal): EvidenceExcerpt {
  const target = session.turns.find(({ idx }) => idx === signal.turnIdx)
  const targetPosition =
    target === undefined ? session.turns.length - 1 : session.turns.indexOf(target)
  const turns = session.turns.slice(Math.max(0, targetPosition - 2), targetPosition + 1)
  return {
...
export function incidentTitle(session: ObservedSession): string {
  const text = [...session.turns]
    .reverse()
    .find(({ role, textRedacted }) => role === 'user' && textRedacted !== null)?.textRedacted
  if (text === undefined || text === null) return 'Repeated agent outcome'
  const compact = text.replace(/\s+/g, ' ').trim()
  return compact.length <= 80 ? compact : `${compact.slice(0, 77)}...`
}

/** Fallback fingerprint for a session with no tool call to key on. */
function embeddingCentroidFingerprint(session: ObservedSession): string {
  const vectors = session.turns.flatMap(({ embedding }) => (embedding === null ? [] : [embedding]))
  if (vectors.length === 0) {
    const text = session.turns.map(({ textRedacted }) => textRedacted ?? '').join('|')
    return sha256(`text:${session.agentId}|${text}`)
  }
  ...
  return sha256(`centroid:${session.agentId}|${centroid.join(',')}`)
}
