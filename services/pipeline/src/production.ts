...
// One pooled connection for the process, created on first use.
let connection: Database | undefined
const database = (): Database => (connection ??= createDatabase())

export const createProductionPipelineControlPlane = (
  input: {
  ...
  const repository = createPostgresPipelineRepository(database())
  const configStore = new PostgresConfigStore({
  ...
  const ledger = new PostgresLedger(database())
  return {
...
export const createProductionPipelineFunctions = (
  input: {
    fallbackConfigs?: ReadonlyMap<string, AgentConfig>
    openAiApiKey?: string
    inngestEventKey?: string
  ...
  const openAiApiKey = required(input.openAiApiKey ?? process.env.OPENAI_API_KEY, 'OPENAI_API_KEY')
