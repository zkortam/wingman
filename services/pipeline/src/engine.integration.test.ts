...
describe('full replay outcome loop', () => {
  it('auto-applies a verified USER preference, confirms it, and isolates a control user', async () => {
    ...
    const controlBefore = canonicalJSON(await configStore.resolve(agentId, controlUser))
    ...
    expect(events.events.filter(({ name }) => name === 'session.observed')).toHaveLength(12)
    expect((await engine.observeSession(first.id)).state).toBe('OPEN')
    const applied = await engine.observeSession(second.id)
    expect(applied.state).toBe('APPLIED')
    expect((await repository.getSnapshot(incidentId(applied.incidentId))).before?.passCount).toBe(0)
    expect((await repository.getSnapshot(incidentId(applied.incidentId))).after?.passCount).toBe(5)
    ...
    expect((await configStore.resolve(agentId, second.userHash)).systemPrompt).toBe('FIXED')
    expect((await configStore.resolve(agentId, first.userHash)).systemPrompt).toBe('BROKEN')
    expect(canonicalJSON(await configStore.resolve(agentId, controlUser))).toBe(controlBefore)
    ...
    expect((await engine.observeSession(confirmation.id)).state).toBe('NO_SIGNAL')
    expect((await repository.getSnapshot(incidentId(applied.incidentId))).outcome?.status).toBe(
      'CONFIRMED',
    )
