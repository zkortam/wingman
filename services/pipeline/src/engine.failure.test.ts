...
describe('pipeline failure containment', () => {
  it('contains a storage outage before an incident can be opened', async () => {
    ...
    vi.spyOn(repository, 'getSession').mockRejectedValue(new Error('database offline'))
    ...
    await expect(engine.observeSession(crypto.randomUUID())).rejects.toThrow('database offline')
  })
