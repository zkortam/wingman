...
describe('retention', () => {
  it('deletes child events while preserving copied incident evidence', async () => {
    ...
    expect(await runRetentionSweep(repository, new Date('2026-03-01T00:00:00.000Z'))).toBe(1)
    expect(database.sessions.size).toBe(0)
    expect(database.signals).toHaveLength(0)
    expect(
      (await repository.getIncident(incidentId)).evidenceExcerpts[0]?.turns[0]?.textRedacted,
...
describe('expiry', () => {
  it('expires an inactive incident after its deadline', async () => {
    ...
    expect(await runExpirySweep(repository, new Date('2026-01-16T00:00:00.000Z'))).toBe(1)
