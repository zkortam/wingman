...
/** Signals for the batch path, where two independent cues are required before anything opens. */
export function detectSignals(input: DetectInput): Signal[] {
  const scored = scoreTurn(input)
  if (scored === null) return []
  return requireConjunction(scored.candidates, PIPELINE_POLICY.signalMinimumConfidence).map(
    (candidate) => toSignal(candidate, input.session.id, scored.turnIdx),
  )
}
...
export function detectLiveSignals(input: DetectInput): Signal[] {
  ...
  return scored.candidates
    .filter(({ confidence }) => confidence > PIPELINE_POLICY.signalMinimumConfidence)
    .map((candidate) => toSignal(candidate, input.session.id, scored.turnIdx))
}

function scoreTurn(input: DetectInput): { candidates: SignalCandidate[]; turnIdx: number } | null {
  ...
  const earlierTexts = userTurns.slice(0, -1).map(({ textRedacted }) => textRedacted)
  const raw: Array<[SignalKind, number, Record<string, string | number | boolean>]> = [
    [
      'RETRY_REQUEST',
    ...
    [
      ...
      {
        comparedSources: earlierTexts.length + (input.session.userRules?.length ?? 0),
      },
    ],
    [
      'ABANDON_RESTART',
...
function toSignal(candidate: SignalCandidate, sessionId: string, turnIdx: number): Signal {
  return {
    sessionId,
    turnIdx,
    kind: candidate.kind,
