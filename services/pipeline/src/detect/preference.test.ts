...
import { isDurablePreference, preferenceStatedConfidence } from './preference.js'

describe('preferenceStatedConfidence', () => {
  it('is highest when several cues agree', () => {
    expect(preferenceStatedConfidence('Just do it, stop asking me to confirm every step.')).toBe(1)
  })
  ...
  it('is zero on a plain task request', () => {
    expect(preferenceStatedConfidence('I want to return the hiking boots I bought.')).toBe(0)
  })

  // The lane split depends on this: an irritated correction is a defect report, not a preference.
...
describe('isDurablePreference', () => {
  ...
  it.each(['Keep this one short please.', 'Just this time, skip the details.', 'Shorter for now.'])(
    'treats %j as scoped to the turn',
    (text) => {
      expect(isDurablePreference(text)).toBe(false)
    },
  )

  // "Always" plus "this time" is ambiguous, and the safe reading is the narrow one: a wrongly.
