...
import type { AgentConfig, AgentDecision, Expectation, Signal, SignalKind } from '@wingman/schema'
...
describe('classifyTurn', () => {
  // The whole posture: Wingman watches turn one and says nothing.
  ...
  // A capability gap outranks a defect: the agent was never given the tool, so a config repair here.
  it('prefers ALERT over FIX when the tool is missing', () => {
    expect(
      classifyTurn(
        input({
          signals: [signal('RETRY_REQUEST', 0.9), signal('RESTATED_CONSTRAINT', 0.9)],
          expectation: expectation('create_shipment'),
          decision: decision('cancel_order'),
        }),
      ).lane,
  ...
  // A wrong answer is a fact and a verbose one is a taste.
  it('prefers FIX over PERSONALIZE when the expectation was also missed', () => {
    expect(
      classifyTurn(
        input({
          signals: [signal('RETRY_REQUEST', 0.8), signal('PREFERENCE_STATED', 0.9)],
          expectation: expectation('create_return'),
          decision: decision('cancel_order'),
  ...
  it('takes no action on dissatisfaction with no expectation to explain it', () => {
    expect(classifyTurn(input({ signals: [signal('RETRY_REQUEST', 0.8)] }))).toEqual({
      lane: 'NONE',
  ...
  it('reports the strongest dissatisfaction confidence', () => {
    expect(
      classifyTurn(
        input({
          signals: [signal('RETRY_REQUEST', 0.6), signal('ABANDON_RESTART', 0.95)],
          expectation: expectation('create_return'),
          decision: decision('cancel_order'),
        }),
      ),
