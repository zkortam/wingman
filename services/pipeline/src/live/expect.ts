 ...
 *
 * The expectation is the only thing that later distinguishes a defect from a capability
 * gap, so it is deliberately allowed to name a tool the agent does not have. Restricting
 * it to the configured tool list would quietly convert every missing capability into
 * either a false repair or silence, which is the failure mode this whole lane exists to
 * avoid.
 *
 * Returns null rather than throwing. Nothing here is worth degrading a live
 * conversation for: with no expectation the classifier simply stays silent.
 */
