...
describe("reviewProposedToolCall", () => {
  ...
  it("bounds a stalled model and rejects malformed decisions", async () => {
    ...
    await expect(reviewProposedToolCall({ model: stalled, config, request, timeoutMs: 10 })).resolves.toMatchObject({
      action: "ALLOW",
      source: "FAIL_OPEN",
    });
    ...
    await expect(reviewProposedToolCall({ model: malformed, config, request })).resolves.toMatchObject({
      action: "ALLOW",
      source: "FAIL_OPEN",
    });
  });
