...
describe('verdict routing', () => {
  it.each([
    ...
    if (verdict === 'CONFIG_DEFECT') expect(result.candidateState).toBe('VERIFIED')
...
async function execute(verdict: Verdict) {
  ...
  const snapshot = await repository.getSnapshot(incidentId(observed.incidentId))
  return {
...
function seedBaseline(database: ReplayDatabase, orgId: string, agentId: string): void {
