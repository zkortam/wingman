...
export class WebSocketAppServerClient implements AppServerClient {
  constructor(
    ...
    if (!endpoint.startsWith('wss://')) throw new Error('Codex App Server endpoint must use wss://')
    if (token.length === 0) throw new Error('Codex App Server bearer token is required')
  }

  async handoff(payload: HandoffPayload): Promise<{ threadId: string }> {
    const rpc = await JsonRpcConnection.connect(this.endpoint, this.token, this.timeoutMs)
    try {
  ...
  async writeAgentsMd(input: { threadId: string; content: string }): Promise<void> {
    const rpc = await JsonRpcConnection.connect(this.endpoint, this.token, this.timeoutMs)
    try {
      await rpc.request('initialize', {
        clientInfo: {
...
class JsonRpcConnection {
  ...
  static connect(endpoint: string, token: string, timeoutMs: number): Promise<JsonRpcConnection> {
  ...
  private receive(raw: string): void {
    ...
    if (parsed.data.error !== undefined) pending.reject(new Error(parsed.data.error.message))
