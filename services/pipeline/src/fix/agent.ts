...
export interface FixAgent {
  propose(input: { prompt: string; iteration: number; timeoutMs: number }): Promise<ConfigDiff>
}
...
export class ReplayFixAgent implements FixAgent {
  ...
  propose(input: { iteration: number }): Promise<ConfigDiff> {
    ...
    if (proposal === undefined) throw new StageError('fix', 'ITERATIONS_EXHAUSTED', false)
...
export class CodexFixAgent implements FixAgent {
  ...
  async propose(input: { prompt: string; timeoutMs: number }): Promise<ConfigDiff> {
    const thread = this.codex.startThread({
      approvalPolicy: 'never',
      networkAccessEnabled: false,
      sandboxMode: 'read-only',
      skipGitRepoCheck: true,
      ...(PIPELINE_MODELS.fix === undefined ? {} : { model: PIPELINE_MODELS.fix }),
    })
