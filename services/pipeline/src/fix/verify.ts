...
export async function proposeAndVerify(
  ...
  const policy = await input.repository.getWritableConfigPolicy(
  ...
  for (
    let iteration = 1;
    iteration <= PIPELINE_POLICY.maxFixIterations;
    iteration += 1
  ) {
    const timeoutMs =
      PIPELINE_POLICY.maxFixTimeMs - (performance.now() - startedAt);
    ...
    const { candidate, diff } = await loggedStage({
      ...
      run: async () => {
        ...
        const candidate = await input.repository.saveCandidate({
          ...
          baseVersionId: await input.repository.getBaseVersionId(
            input.incident.agentId,
          ),
...
async function runPositiveSuite(
  ...
  const assertions = await input.repository.listPositiveAssertions(
    input.incident.agentId,
  );
