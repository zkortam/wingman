...
import { clusterIdentity, evidenceExcerpt, incidentTitle } from './cluster/index.js'
...
export interface PipelineEngine {
  observeSession(sessionId: string): Promise<{ incidentId: string | null; state: string }>
  resumeIncident(incidentId: string): Promise<{ incidentId: string; state: string }>
}
...
export function createPipelineEngine(input: {
  ...
  /** Parks an incident, but only from a state the machine allows it to be parked from. */
  const park = async (incident: IncidentRecord, reason: string): Promise<IncidentRecord> => {
    if (!canTransition(incident.state, 'PARKED')) {
      if (TERMINAL.has(incident.state)) return incident
    ...
    async observeSession(sessionId) {
      ...
      try {
        ...
        const baselines = await input.repository.getBaselines(
          session,
          new Date(now().getTime() - PIPELINE_POLICY.baselineWindowDays * 24 * 60 * 60 * 1_000),
        )
        ...
        const [primary] = [...signals].sort((left, right) => right.confidence - left.confidence)
        if (primary === undefined) return { incidentId: null, state: 'NO_SIGNAL' }
        ...
        let incident = await input.repository.createOrJoinIncident({
          ...
          expiresAt: new Date(
            now().getTime() + PIPELINE_POLICY.incidentExpiryDays * 24 * 60 * 60 * 1_000,
          ).toISOString(),
        })
        tracked = incident
        logger.write({
        ...
        if (incident.state === 'OPEN') {
          if (
            ...
            incident = await input.repository.updateIncident(incident.id, 'OPEN', {
              state: 'PARKED',
