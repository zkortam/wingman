...
export function createReplayReadRepository(
  database: ReplayDatabase,
): ReplayReadRepository {
  ...
  function getAssertion(id: string): Promise<Assertion> {
    return Promise.resolve(
      structuredClone(required(database.assertions, id, "Assertion")),
    );
  }

  function getCandidate(id: string): Promise<Candidate> {
    return Promise.resolve(
      structuredClone(required(database.candidates, id, "Candidate")),
    );
  }
  ...
  function latestCandidate(
    ...
    const candidates = [...database.candidates.values()]
      .filter(
        (candidate) =>
          candidate.incidentId === incidentId && candidate.attempt === attempt,
      )
      .sort((left, right) => right.iteration - left.iteration);
    return Promise.resolve(
      candidates[0] === undefined ? null : structuredClone(candidates[0]),
    );
  }
  ...
  function getOutcomeForIncident(incidentId: string): Promise<Outcome | null> {
    const outcome = [...database.outcomes.values()]
      ...
      .at(-1);
    return Promise.resolve(
      outcome === undefined ? null : structuredClone(outcome),
    );
  }
  ...
  async function getSnapshot(incidentId: string): Promise<PipelineSnapshot> {
    ...
    const runs = [...database.runs.values()].filter(
      (run) =>
        run.incidentId === incidentId && run.attempt === incident.attempt,
    );
    return {
      incident,
      assertion:
        incident.assertionId === null
          ? null
          : await getAssertion(incident.assertionId),
  ...
  return {
    ...
    async getBaselines(session, since) {
      ...
      const user = prior.filter(
        ({ userHash }) => userHash === session.userHash,
      );
      return rates(
    ...
    countInFlight(agentId) {
      return Promise.resolve(
        [...database.incidents.values()].filter(
          (incident) =>
            incident.agentId === agentId && IN_FLIGHT.includes(incident.state),
        ).length,
      );
    },
    getIncident,
    findIncident(agentId, key) {
      ...
      return Promise.resolve(
        incident === undefined ? null : structuredClone(incident),
      );
    },
    getAssertion,
    ...
    async findPendingOutcome(session) {
      const outcome = [...database.outcomes.values()].find((candidate) => {
        if (
          ...
          !candidate.appliedTo.includes(session.userHash)
        )
          return false;
        return (
          database.incidents.get(candidate.incidentId)?.fingerprint ===
    ...
    listOutcomes(orgId) {
      ...
      return Promise.resolve(
        structuredClone(
          [...database.outcomes.values()].filter(({ incidentId }) =>
            ids.has(incidentId),
          ),
        ),
      );
    ...
    getHandoff(incidentId) {
      return Promise.resolve(
        structuredClone(database.handoffs.get(incidentId) ?? null),
      );
    },
    async getWritableConfigPolicy(agentId) {
      return structuredClone(
        required(database.writablePolicies, agentId, "Writable policy"),
      );
    },
    ...
    async getIncidentDiff(incidentId) {
      const incident = await getIncident(incidentId);
      return (
        (await latestCandidate(incidentId, incident.attempt))?.diff ?? null
      );
    },
  };
}
