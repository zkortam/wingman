...
export async function processIncident(input: ContinueInput): Promise<IncidentRecord> {
  ...
  const { gate } = await loggedStage({
    ...
    stage: 'gate',
    run: async () => {
      const live = await input.configStore.resolve(input.incident.agentId, input.session.userHash)
    ...
    outcome: ({ gate }) => (gate.requiresHumanReview ? 'HUMAN_REVIEW' : gate.decision.verdict),
  })
  ...
  const incident = await input.repository.updateIncident(input.incident.id, 'CLUSTERED', {
    state: 'CLASSIFIED',
    verdict: gate.decision.verdict,
    verdictConfidence: gate.decision.confidence,
    verdictEvidence: gate.decision.evidence,
