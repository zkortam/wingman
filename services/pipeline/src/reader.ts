...
export function createPipelineReader(
  ...
  return {
    incidentInOrg(orgId, incidentId) {
      return repository.incidentInOrg(orgId, incidentId)
    },
    async listIncidents(orgId, options) {
      // Incident rows only: the summary DTO reads nothing from a full snapshot.
      const incidents = await repository.listIncidents(orgId, options)
