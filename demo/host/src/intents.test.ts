...
describe("expectedIntent", () => {
  ...
  it("does not steal cancel, lookup, return, or the Germany gap", () => {
    ...
    expect(expectedIntent("can you ship this to Germany instead")).toBe(
...
describe("wantsRepairedReschedule", () => {
  it("still catches the paraphrases after a live fix", () => {
    expect(wantsRepairedReschedule("make it aug 24", "2026-08-23", "2026-08-26")).toBe(
      true,
    );
    expect(wantsRepairedReschedule("change the delivery date", "2026-08-23")).toBe(true);
    expect(wantsRepairedReschedule("I need this to arrive later", "2026-08-23")).toBe(
      true,
    );
  });

  it("does not treat tracking or a doorstep note as another reschedule", () => {
    expect(wantsRepairedReschedule("where's my package", "2026-08-23", "2026-08-26")).toBe(
      false,
    );
    expect(wantsRepairedReschedule("where's my delivery", "2026-08-23", "2026-08-26")).toBe(
      false,
    );
