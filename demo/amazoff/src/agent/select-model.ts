...
export function renderPrompt(utterance: string, config: AgentConfig): string {
  const tools = Object.entries(config.tools)
    ...
    .join('\n')
  const rules = config.rules.map((rule, index) => `${String(index + 1)}. ${rule}`).join('\n')
  return [
    config.systemPrompt,
    '',
    ...
    '',
    // Without this a real model often reaches for a lookup first, which is sensible behaviour but.
