...
interface RespondInput {
  ...
  customerId: string
  config: AgentConfig
  /** A tool choice already made elsewhere, normally by a language model. */
  selection?: ToolSelection | null
}

/** Amazoff's support agent. */
export class AmazoffAgent {
  ...
  respond({ utterance, customerId, config, selection: given }: RespondInput): AgentReply {
    // Greetings and thanks need no tool, and treating them as unmet requests would both read as broken.
