...
const session = (toolCalls: SessionInput['turns'][number]['toolCalls']): SessionInput => ({
...
describe('taskFingerprint', () => {
  it('hashes agentId, first tool name, and its object type', () => {
    expect(
      taskFingerprint(session([{ name: 'export_records', args: { objectType: 'opportunity' } }])),
  ...
  it("falls back to 'unknown' when no object type is present", () => {
    expect(taskFingerprint(session([{ name: 'export_records', args: {} }]))).toBe(
      '9f3838fdd4b83b8d6a45ee79a8f78c2546670d4169a9b269b4bd1ca6a7fe1ee5',
    )
  })
  ...
  // §7's prose: with no tool decision and embeddings cut, "sessions with no tool decision simply do.
...
describe('assertionIdentity', () => {
  ...
  // "Fingerprint buckets; identity identifies." A differing identity must produce a different key so.
