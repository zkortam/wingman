...
const expectation = (definition: Expectation['definition']): Expectation => ({
  id: randomUUID(),
...
describe('ExpectationSchema', () => {
  it('accepts an expectation built from an assertion definition', () => {
    expect(() =>
      ExpectationSchema.parse(expectation({ kind: 'TOOL_CALLED', tool: 'create_return' })),
    ).not.toThrow()
  })
...
describe('isExpectationSupported', () => {
  it('is true when the agent has the implied tool', () => {
    expect(
      isExpectationSupported(expectation({ kind: 'TOOL_CALLED', tool: 'create_return' }), config),
    ).toBe(true)
  })

  // The ALERT/FIX split turns on this.
  it('is false when the implied tool does not exist', () => {
    expect(
      isExpectationSupported(expectation({ kind: 'TOOL_CALLED', tool: 'create_shipment' }), config),
    ).toBe(false)
  })
  ...
  it('is not satisfied by an inherited tools property', () => {
    expect(
      isExpectationSupported(expectation({ kind: 'TOOL_CALLED', tool: 'constructor' }), config),
    ).toBe(false)
  })
})
...
describe('isExpectationMet', () => {
  ...
  it('is missed when the agent calls nothing at all', () => {
    expect(isExpectationMet(returnExpected, { toolCalls: [], text: 'Sure!' }, context)).toBe(false)
  })
})

describe('expectationTool', () => {
  it('names the tool for tool expectations', () => {
    expect(expectationTool(expectation({ kind: 'TOOL_CALLED', tool: 'create_return' }))).toBe(
      'create_return',
    )
  })
  ...
  it('is null for output-shape expectations', () => {
    expect(
      expectationTool(expectation({ kind: 'OUTPUT_MATCHES_RULE', rule: 'be concise' })),
    ).toBeNull()
  })
})
