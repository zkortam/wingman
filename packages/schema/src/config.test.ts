...
import { applyDiff, ConfigDiffSchema, diffConfigs, isConfigPathWritable } from './config.js'
...
describe('ConfigDiff', () => {
  ...
  it('matches writable paths with explicit wildcard semantics', () => {
    expect(isConfigPathWritable('tools.export_records.description', 'tools.*.description')).toBe(
      true,
    )
    expect(
      isConfigPathWritable('tools.export_records.description.extra', 'tools.*.description'),
    ).toBe(false)
    expect(isConfigPathWritable('retrieval.reranker.model', 'retrieval.*')).toBe(true)
