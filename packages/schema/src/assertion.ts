...
import { JsonPrimitiveSchema, type JsonPrimitive, type JsonValue } from './json.js'
...
export const AssertionDefinitionSchema = z.discriminatedUnion('kind', [
  z.object({ kind: z.literal('TOOL_CALLED'), tool: z.string().min(1) }).strict(),
  z
    .object({
      kind: z.literal('TOOL_ARG_EQUALS'),
      tool: z.string().min(1),
      arg: z.string().min(1),
      expected: z.union([JsonPrimitiveSchema, z.array(JsonPrimitiveSchema), ContextRefSchema]),
    })
    .strict(),
  z.object({ kind: z.literal('OUTPUT_MATCHES_RULE'), rule: z.string().min(1) }).strict(),
])
...
export function isDecidableAtToolBoundary(assertion: AssertionDefinition): boolean {
...
export function evaluateAssertion(
  ...
  return decision.toolCalls
    ...
    .some(({ args }) => assertionEqual(readPath(args, assertion.arg), expected))
}

export function assertionEqual(left: JsonValue | undefined, right: JsonValue | undefined): boolean {
  ...
  if (Array.isArray(left) || Array.isArray(right)) {
    ...
    if (left.every(isPrimitive) && right.every(isPrimitive)) {
      return [...left]
        ...
        .every((value, index) => value === [...right].sort(comparePrimitive)[index])
    }
  ...
  return (
    ...
    keys.every((key, index) => key === rightKeys[index] && assertionEqual(left[key], right[key]))
  )
}

function resolveContext(ref: ContextRef, context: AssertionContext): JsonValue | undefined {
  switch (ref.$ref) {
...
/** Reads a dotted path out of tool arguments. */
