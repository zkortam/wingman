...
import { LiveClassificationSchema, NO_RECOVERY, RecoveryDirectiveSchema } from './live.js'
...
describe('LiveClassificationSchema', () => {
  ...
  // A satisfied turn must be representable, or the classifier gets pushed toward inventing a lane.
  ...
  it('requires a rationale on every lane', () => {
    expect(() => LiveClassificationSchema.parse({ lane: 'NONE', rationale: '' })).toThrow()
  })
})
...
describe('RecoveryDirectiveSchema', () => {
  ...
  it('rejects an empty user-visible message rather than showing a blank turn', () => {
    expect(() => RecoveryDirectiveSchema.parse({ ...NO_RECOVERY, message: '' })).toThrow()
  })
