...
const preference = (rule: string, state: PreferenceRule['state'] = 'ACTIVE'): PreferenceRule => ({
  id: randomUUID(),
  orgId: randomUUID(),
  agentId: randomUUID(),
...
describe('PreferenceRuleSchema', () => {
  ...
  it('bounds the rule length so a preference cannot become a second prompt', () => {
    expect(() => PreferenceRuleSchema.parse(preference('x'.repeat(201)))).toThrow()
  })
})
...
describe('applyPreferences', () => {
  it('appends active preferences to the resolved config rules', () => {
    expect(
      applyPreferences(config, [preference('Do not ask for confirmation before acting.')]).rules,
    ).toEqual([
  ...
  it('ignores revoked preferences', () => {
    expect(applyPreferences(config, [preference('Be brief.', 'REVOKED')]).rules).toEqual(
      config.rules,
    )
  })
  ...
  it('does not duplicate a rule the config already states', () => {
    expect(
      applyPreferences(config, [preference('Never disclose internal order notes.')]).rules,
