...
describe("CapabilityRequestSchema", () => {
  it("requires hashed requesters so a demand count is never a customer list", () => {
    expect(() =>
      CapabilityRequestSchema.parse(request(["ledgerline-user-01"])),
    ).toThrow();
  });

  it("accepts a request with no nameable tool", () => {
    expect(() =>
      CapabilityRequestSchema.parse({ ...request([]), impliedTool: null }),
    ).not.toThrow();
  });
});
...
describe("capabilityDemand", () => {
  ...
  // One frustrated user asking six times is one unit of demand. Counting attempts
  // would let a single loop outrank a genuinely popular gap.
  it("counts a repeatedly asking user once", () => {
    expect(
      capabilityDemand(request(["a".repeat(32), "a".repeat(32), "a".repeat(32)])),
    ).toBe(1);
  });
...
describe("capabilityKey", () => {
  ...
  // The point of the lane is the count, so differently worded requests for the same
  // gap must collide rather than becoming three separate roadmap items.
