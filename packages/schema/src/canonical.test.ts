...
// DATA-MODEL.md §8. The SDK verifies signatures in the customer's process against
// this exact byte string. A producer/verifier mismatch rejects every version and
// silently falls back to base, which looks like "the product does nothing" rather
// than like a bug — so these are pinned as literals, not compared to a reimplementation.
const AWKWARD: ReadonlyArray<readonly [string, unknown, string]> = [
  ...
  // Sorting is by UTF-16 code unit, NOT locale collation. `localeCompare` would
  // order these {a, B, c} and produce different bytes for the same config.
...
describe("canonicalJSON", () => {
  ...
  it("sorts keys by code unit rather than locale collation", () => {
    ...
    // A verifier using localeCompare would order these a, B, c, sign different
    // bytes, reject every version, and fall back to base config silently.
    expect(
      ["B", "a", "c"].sort((left, right) => left.localeCompare(right)),
    ).toEqual(["a", "B", "c"]);
  });
  ...
  // Documented rather than desired: §8 says "numbers via JSON.stringify", and that
  // maps non-finite values to null. Pinned so a future change to encode() is a
  // deliberate decision instead of a silent shift in the signed bytes.
  it.each([
    ["NaN", Number.NaN],
    ["Infinity", Number.POSITIVE_INFINITY],
  ])("encodes %s as null", (_label, value) => {
...
describe("signConfig", () => {
  ...
  it("changes when any component changes", () => {
    ...
    expect(
      signConfig("key", "agent", 2, { ...config, systemPrompt: "other" }),
    ).not.toBe(base);
  });
});
