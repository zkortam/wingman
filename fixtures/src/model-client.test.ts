...
describe('CassetteModelClient', () => {
  it('records once and replays without calling the model boundary', async () => {
    ...
    const recorder = new CassetteModelClient({
      store: new CassetteStore({ directory, mode: 'record' }),
      record: realModel,
    })
    expect(await recorder.generate(request)).toEqual({ text: 'recorded' })

    const forbiddenNetwork = vi.fn(async () => {
      throw new Error('network called during replay')
    })
    const replay = new CassetteModelClient({
      store: new CassetteStore({ directory, mode: 'replay' }),
      record: forbiddenNetwork,
    })
