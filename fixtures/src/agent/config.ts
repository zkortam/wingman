...
export const BASE_RUNTIME_CONFIG: RuntimeConfig = {
  ...
  tools: [
    { name: 'search_records', description: 'Search CRM opportunities using the supplied query and filters.' },
    {
      name: 'export_records',
      description: "Export CRM opportunities. Pass the caller's active view filters so the export matches the visible view.",
    },
...
export const runtimeConfig = (value: unknown): RuntimeConfig => {
  ...
  if (!Array.isArray(candidate.tools) || !candidate.tools.every(isRuntimeTool)) return BASE_RUNTIME_CONFIG
  if (!Array.isArray(candidate.rules) || !candidate.rules.every((rule) => typeof rule === 'string')) {
