...
describe('resetDemo', () => {
  it('validates the committed offline cohort without network', async () => {
    await expect(resetDemo()).resolves.toEqual({ sessions: 50, affected: 12, pipelineVerified: false })
  })
  ...
  it('replays every session and verifies the live candidate when an endpoint is supplied', async () => {
    ...
    const fetcher = vi.fn(async (input: string | URL | Request) => {
      ...
      return new Response(JSON.stringify([{ id: 'OC-1042', users: 12, state: 'CANDIDATE' }]), { status: 200 })
    })
    await expect(resetDemo({ baseUrl: 'https://outcome.test', fetcher })).resolves.toMatchObject({ pipelineVerified: true })
    expect(posted).toBe(50)
  })

  it('rebases committed timestamps to reset time without changing their spacing', () => {
    const sessions = [{
      id: 's1', agentId: 'agent', userId: 'user', personaId: 'p1', defectId: 'OC-001', affected: true,
      startedAt: '2025-01-01T11:00:00.000Z', endedAt: '2025-01-01T11:02:00.000Z',
