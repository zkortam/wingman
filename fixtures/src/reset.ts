...
const shiftIso = (value: string, deltaMs: number): string =>
...
export const resetDemo = async (options: ResetOptions = {}): Promise<ResetReport> => {
  ...
  await new CassetteStore({ directory: cassetteDirectory, mode: 'replay' }).preflight(
    requiredCassetteRequests,
  )
  ...
  if (sessions.length !== 50 || affected !== 12)
    throw new Error(
      `Invalid cohort: ${String(sessions.length)} sessions, ${String(affected)} affected`,
    )
  ...
  const responses = await Promise.all(
    sessions.map((session) =>
      fetcher(`${options.baseUrl}/v1/events`, {
        method: 'POST',
  ...
  if (responses.some((response) => response.status !== 202))
    throw new Error('Session replay failed')
...
const run = async (): Promise<void> => {
  ...
  const report = await resetDemo(
    process.env.WINGMAN_API_URL ? { baseUrl: process.env.WINGMAN_API_URL } : {},
  )
  process.stdout.write(
    `Demo reset: ${String(report.sessions)} sessions, ${String(report.affected)} affected, ${report.pipelineVerified ? 'pipeline verified' : 'offline fixtures verified'}, ${String(Math.round(performance.now() - started))}ms\n`,
  )
}
