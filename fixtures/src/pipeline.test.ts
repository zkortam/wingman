...
const readDefect = async (id: string): Promise<{ expected: { verdict: string } }> =>
  JSON.parse(await readFile(resolve(import.meta.dirname, `../defects/${id}.json`), 'utf8')) as {
...
describe('pipeline replay fixtures', () => {
  it('pins all four fixture verdicts without per-run tuning', async () => {
    ...
    for (const [id, verdict] of expected)
      expect((await readDefect(id)).expected.verdict).toBe(verdict)
  })
  ...
  it('demonstrates fail-before and pass-after with zero executions', async () => {
    ...
    const messages = [
      { role: 'user', text: 'Export these', context: { viewFilters: { stage: 'Negotiation' } } },
    ] as unknown as Turn[]
    const broken = {
      ...BASE_RUNTIME_CONFIG,
      tools: BASE_RUNTIME_CONFIG.tools.map((tool) =>
        tool.name === 'export_records'
          ? { ...tool, description: 'Exports records from the current object.' }
          : tool,
      ),
    ...
    const run = async (config: AgentConfig): Promise<{ passes: number; executions: number }> => {
      const results = await Promise.all(
        Array.from({ length: 5 }, (_, sample) =>
          runner.runTurn({ config, messages, sample, intercept: () => 'INTERCEPT' }),
        ),
      )
      return {
        passes: results.filter(
          (result) =>
            JSON.stringify(
              (result.toolCalls[0] as unknown as { args: { filters: unknown } }).args.filters,
