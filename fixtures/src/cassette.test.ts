...
describe('CassetteStore', () => {
  it('replays five recorded responses without touching the network', async () => {
    ...
    expect(responses).toEqual([{ sample: 0 }, { sample: 1 }, { sample: 2 }, { sample: 3 }, { sample: 4 }])
    expect(record).not.toHaveBeenCalled()
    expect(JSON.parse(await readFile(join(directory, `${key}.json`), 'utf8'))).toMatchObject({ key })
  })
  ...
  it('fails preflight before a demo starts when a key is absent', async () => {
    ...
    await expect(store.preflight([{ model: 'demo', messages: [] }])).rejects.toThrow('Missing cassette')
  })
  ...
  it('preflights every committed demo cassette', async () => {
    ...
    const responses = await Promise.all(Array.from({ length: 5 }, (_, sample) => store.response(requiredCassetteRequests[0], sample, async () => null)))
