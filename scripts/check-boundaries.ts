...
const walk = async (directory: string): Promise<string[]> => {
  ...
  return nested.flat().filter((path) => sourceExtensions.has(extname(path)) && !path.includes('.test.'))
}
...
const files = (await Promise.all(sourceRoots.map((directory) => walk(join(root, directory))))).flat()
...
const resolveLocal = (from: string, target: string): string | undefined => {
  ...
  return [`${base}.ts`, `${base}.tsx`, join(base, 'index.ts'), join(base, 'index.tsx')].find((candidate) => fileSet.has(candidate))
}
...
const visit = (file: string, path: string[]): void => {
  if (visiting.has(file)) {
    ...
    failures.push(`circular dependency: ${path.slice(start).concat(file).map((entry) => relative(root, entry)).join(' -> ')}`)
    return
  }
