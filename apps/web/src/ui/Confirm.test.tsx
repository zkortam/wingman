...
describe('Confirm', () => {
  it('uses an accessible modal and returns cancellation', async () => {
    const cancel = vi.fn()
    render(<Confirm body="Global impact" onCancel={cancel} onConfirm={vi.fn()} title="Apply globally?" />)
  ...
  it('keeps keyboard focus inside the destructive confirmation', async () => {
    render(<Confirm body="Global impact" onCancel={vi.fn()} onConfirm={vi.fn()} title="Apply globally?" />)
