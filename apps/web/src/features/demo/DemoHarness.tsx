  ...
  return (
    <section className="demo-pane" aria-label={`${kind.toLowerCase()} demo window`}>
      <header className="demo-header">
        ...
        <span className="demo-version" data-changed={state.version === 'v2'}>
          config {state.version}
        </span>
      </header>
      <div className="demo-transcript" aria-live="polite">
        {state.messages.length === 0 ? (
          <span className="muted">Stage = Negotiation, 10 visible opportunities</span>
        ) : null}
        {state.messages.map((message, index) => (
          <div className="demo-message" data-role={message.role} key={index}>
            {message.text}
          </div>
        ))}
      </div>
      <div className="demo-composer">
        <label className="screen-reader-only" htmlFor={`${kind}-message`}>
          Message
        </label>
        <input
          id={`${kind}-message`}
          ...
          value={input}
        />
        <button className="primary-button" disabled={!input.trim()} onClick={send} type="button">
          Send
        </button>
      </div>
    </section>
