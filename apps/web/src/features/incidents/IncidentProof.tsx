...
interface IncidentProofProps {
  ...
  client?:
    | Pick<typeof apiClient, 'apply' | 'dismiss' | 'handoff' | 'reopen' | 'revert' | 'getIncident'>
    | undefined
}
...
export const IncidentProof = ({
  ...
  const performApply = async (): Promise<void> => {
    ...
    try {
      ...
      setIncident((value) => ({
        ...
        confirmation: { status: 'PENDING', detail: 'Confirmation window ends in 24h' },
      }))
  ...
  const dismiss = async (): Promise<void> => {
    ...
    try {
      ...
      setIncident((value) => ({
        ...value,
        state: 'DISCARDED',
        stateReason: 'Dismissed by operator',
      }))
  ...
  const completeConfirmation = (): void => {
    void client
      ...
      .then((next) => {
        if (next.state === 'CONFIRMED' || next.confirmation?.status === 'CONFIRMED')
          setIncident(next)
      })
      .catch(() => undefined)
  }
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent): void => {
      ...
      // `event.key` is still "a" while Cmd or Ctrl is held, so without this the browser's own Select.
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <button
              disabled={!previousId}
              onClick={() => previousId && router.push(`/incidents/${previousId}`)}
              type="button"
            >
              Previous
            </button>
            <button
              disabled={!nextId}
              onClick={() => nextId && router.push(`/incidents/${nextId}`)}
              type="button"
            >
              Next
            </button>
            <button onClick={() => setExpandedEvidence((value) => !value)} type="button">
              {expandedEvidence ? 'Collapse evidence' : 'Expand evidence'}
            </button>
              ...
              <>
                <button
                  onClick={() =>
                    void client
                      ...
                      .then(() => setToast('Handoff payload copied'))
                ...
                <button
                  onClick={() =>
                    void client
                      .handoff(incident.id)
                      .then(() => setToast('Handoff resent'))
                      .catch(() => setToast('Resend failed'))
                  }
                >
                  Resend
                </button>
            ...
            {actions.includes('dismiss') && !actions.includes('apply') ? (
      ...
      />
      <div aria-live="polite" className="incident-status" data-tone={presentation.tone}>
        {presentation.status}
      </div>
      <div className="proof">
        ...
        {presentation.show.verdict && incident.verdict ? (
          <ProofBlock label="CLASSIFIED">
            <Verdict verdict={incident.verdict} />
          </ProofBlock>
        ) : null}
        {presentation.show.assertion && incident.assertion ? (
          <ProofBlock label="ASSERTION">
            <Assertion assertion={incident.assertion} />
          </ProofBlock>
        ) : null}
        {presentation.show.before && incident.before ? (
          <ProofBlock label="BEFORE">
            {incident.state === 'ASSERTED' ? (
              <span className="muted">Running verification</span>
            ) : (
              <Dots n={incident.before.n} passCount={incident.before.passCount} />
            )}
          ...
          <ProofBlock label="CHANGE">
            {incident.change.bytes > 4_096 ? (
              <span className="muted">
                Diff exceeds the 4 KB cap. Human approval required regardless of scope.
              </span>
            ) : (
          ...
          <ProofBlock label="AFTER">
            <div className="dots-line">
              ...
              <span className="suite-count">
                positive suite {incident.after.positiveSuitePassed}/
                {incident.after.positiveSuiteTotal} green
              </span>
            </div>
          </ProofBlock>
        ) : null}
        {incident.stateReason ? (
          <ProofBlock label="REASON">
            <span className="park-reason">{incident.stateReason}</span>
          </ProofBlock>
        ) : null}
        {presentation.show.handoff && incident.handoff ? (
          <ProofBlock label="HANDOFF">
            <pre className="diff">{incident.handoff}</pre>
          </ProofBlock>
        ) : null}
        {presentation.show.confirmation && incident.confirmation ? (
          <ProofBlock label="CONFIRMATION">
            <div className="confirmation-line">
              <span>{incident.confirmation.detail}</span>
            </div>
          </ProofBlock>
        ) : null}
        {actions.includes('apply') ? (
          <ProofBlock label="SCOPE">
            <div className="scope-line">
              <span>
                {incident.scope === 'GLOBAL' ? 'All users' : `${incident.users} affected users`}
              </span>
              <div className="scope-actions">
                ...
                {actions.includes('apply') ? (
                  <button className="primary-button" disabled={busy} onClick={apply}>
                    {busy ? 'Applying' : incident.scope === 'GLOBAL' ? 'Apply globally' : 'Apply'}
                  </button>
                ) : null}
              </div>
            </div>
      ...
      {confirm ? (
        <Confirm
          body={
            confirm === 'global'
              ? 'This changes the active config for every user.'
              : 'This restores the previous config for the affected user.'
          }
