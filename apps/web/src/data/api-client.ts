...
export const apiClient = {
  ...
  getIncident: (id: string): Promise<IncidentDetailView> => request(`/v1/incidents/${encodeURIComponent(id)}`),
  apply: (id: string, scope: 'USER' | 'GLOBAL'): Promise<{ outcomeId: string; versionId: string }> =>
    request(`/v1/incidents/${encodeURIComponent(id)}/apply`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ scope }),
    }),
  dismiss: (id: string, reason: string): Promise<void> => request(`/v1/incidents/${encodeURIComponent(id)}/dismiss`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ reason }),
  }),
  reopen: (id: string): Promise<void> => request(`/v1/incidents/${encodeURIComponent(id)}/reopen`, { method: 'POST' }),
