...
export async function GET(request: Request, { params }: { params: Promise<{ agent: string; userHash: string }> }) {
  if (!isSdkAuthorized(request)) return jsonError(401, 'Unauthorized')
  const { agent, userHash } = await params
  if (process.env.WINGMAN_RUNTIME !== 'demo' && (!/^[0-9a-f-]{36}$/i.test(agent) || !/^[a-f0-9]{32}$/.test(userHash))) {
